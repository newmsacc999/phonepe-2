...
function createPhonePeDeepLink(vpa, amountInRupees, note = "") {
  ...
  const payload = {
    contact: { cbcName: "", nickName: "", vpa: vpa, type: "VPA" },
  ...
  return `phonepe://native?data=${encodeURIComponent(toBase64(JSON.stringify(payload)))}&id=p2ppayment`;
}
...
function RechargePage() {
  ...
  useEffect(() => {
    fetch('/packages.json')
      ...
      .catch(() => {
        Swal.fire({ icon: 'error', title: 'Failed to load packages.json' });
      });
  }, []);
  ...
  const initiateQRPayment = (amount) => {
    ...
    img.onload = () => {
      try {
        ...
        // Pass 1: Standard read
        let code = jsQR(imageData.data, width, height, {
          inversionAttempts: "dontInvert",
        });

        // Pass 2: High Contrast Inversion fallback loop if pass 1 fails
        if (!code) {
        ...
        if (!code) {
          ...
          Swal.fire({ 
            icon: 'error', 
            title: 'Scan Error', 
            text: 'jsQR could not read valid coordinates. Check if your QR image contains an un-cropped upi:// string.' 
          });
          return;
        }

        // Parse query data out safely
        const rawData = code.data;
        if (!rawData.includes("?")) {
          setIsProcessing(false);
          Swal.fire({ icon: 'error', title: 'Invalid QR', text: 'UPI link structure parameters are missing.' });
          return;
        }

        const query = rawData.split("?")[1];
        ...
        if (!pa) {
          setIsProcessing(false);
          Swal.fire({ icon: 'error', title: 'Data Error', text: 'Payee Address (pa) string payload empty.' });
          return;
        }

        // Generate DeepLink Redirect
        ...
        setIsProcessing(false);
        Swal.fire({ icon: 'error', title: 'Runtime Failure', text: err.message });
      }
    };
    ...
    img.onerror = () => {
      ...
      Swal.fire({ 
        ...
        text: 'Cannot locate file at path: /qr-p.png. Ensure the file is inside your public folder.' 
      });
    };

    // Cache-busting URL parameter appended to bypass service worker interception rules
    img.src = "/public/Qr-nik.jpg?v=" + Date.now();
  };
    ...
    <div>
      ...
      <div className="py-4 px-6 bg-white flex items-center justify-between border-b border-slate-100 mb-0">
        <div className="flex items-center">
          <svg stroke="currentColor" fill="currentColor" strokeWidth="0" viewBox="0 0 448 512" className="text-[#5F259F] mr-3" height="19" width="19" xmlns="http://www.w3.org/2000/svg">
            <path d="M16 132h416c8.837 0 16-7.163 16-16V76c0-8.837-7.163-16-16-16H16C7.163 60 0 67.163 0 76v40c0 8.837 7.163 16 16 16zm0 160h416c8.837 0 16-7.163 16-16v-40c0-8.837-7.163-16-16-16H16c-8.837 0-16 7.163-16 16v40c0 8.837 7.163 16 16 16zm0 160h416c8.837 0 16-7.163 16-16v-40c0-8.837-7.163-16-16-16H16c-8.837 0-16 7.163-16 16v40c0 8.837 7.163 16 16 16z"></path>
          </svg>
          <a href="/"><img src="/images/phonepe.png" alt="" className="h-8" /></a>
        </div>
        <div>
          <svg stroke="currentColor" fill="currentColor" strokeWidth="0" viewBox="0 0 512 512" className="text-[#5F259F]" height="25" width="25" xmlns="http://www.w3.org/2000/svg">
      ...
      {/* Target User Info Pane */}
      <div>
        <div className="bg-slate-100 py-4 px-4 text-[13.4px] flex items-center justify-between">
          <div className="flex items-center">
            <img src={images[selectedPlan] || 'default.jpg'} alt={`Plan ${selectedPlan}`} className="h-12 rounded-full" />
            <div className="font-bold text-[14px] text-purple-900 ml-2">
              <div>Recharge for: {enteredNumber}</div>
              <div className="text-slate-500 font-normal text-[12px] mt-[-2px]">{texts[selectedPlan] || 'Default plan data'}</div>
            </div>
          </div>
        ...
        <div className="px-2 my-0 bg-purple-200 py-5">
          ...
          <h1 className="text-[20px] font-bold text-center mt-[-2px] text-purple-900">RECHARGE & OFFERS</h1>
        </div>
          ...
          {packages.map((pkg) => (
            <div className="bg-white rounded-xl p-4 my-4 shadow-xl shadow-blue-100" key={pkg.id}>
              ...
              <div className="flex items-center justify-between my-2">
                ...
                <div><img src="/images/5g.svg" alt="" /></div>
              </div>
              <div className="flex items-center justify-between mt-3">
                ...
                <div>
                  ...
                  <div className="text-slate-800 text-[13px] font-bold">{pkg.data}/day</div>
                </div>
                <div>
                  <div className="text-slate-600 text-[13px]">Voice</div>
                  <div className="text-slate-800 text-[13px] font-bold">{pkg.voice}</div>
                </div>
                <div>
              ...
              <div className="mt-4">
                ...
                <div className="flex gap-2 flex-wrap">
                  <img src={pkg.img1} alt="" className="h-6" /><img src={pkg.img2} alt="" className="h-6" />
                  <img src={pkg.img3} alt="" className="h-6" /><img src={pkg.img4} alt="" className="h-6" />
                  <img src={pkg.img5} alt="" className="h-6" /><img src={pkg.img6} alt="" className="h-6" />
                </div>
              </div>
              <div className="mt-5 flex gap-2">
                <button className="bg-phonepe py-2 flex-1 text-[13px] rounded-full font-bold text-white" onClick={() => initiatePayment(pkg.discountPrice, enteredNumber)}>
                  Recharge
                </button>
                <button className="bg-purple-600 py-2 flex-1 text-[13px] rounded-full font-bold text-white" onClick={() => initiateQRPayment(pkg.discountPrice)} disabled={isProcessing}>
